...
set)"). So this module ships with:
  - A CalibrationModel that CAN be fit against labeled data once it
    ...
    of the stack).
  - An explicit, clearly-labeled 50/50-average fallback used only when
    no fitted calibration model is available, so it's never confused
    with a real calibrated score.
"""
...
def aggregate(text: str, calibration: CalibrationModel = None) -> dict:
    ...
    Returns:
        {
          ...
          "raw_score": float,
          "confidence": float,
          "calibrated": bool,  # False if this used the 50/50 fallback
        }
    ...
    """
    try:
        signal_1_score = stylometric.score(text)
